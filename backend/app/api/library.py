...
from ..models import CorpusEntry, CorpusLink, LibraryComment, Material, Organ, GenomeDraft
...
class ChimeraGenRequest(BaseModel):
    model: Optional[str] = None


def _resolve_organ_names_to_ids(db: Session, by_bank: dict, source_entry_id: str) -> dict[str, list[str]]:
    """Match weaver-returned organ names against canonical organs by case-insensitive
    name match. Unknown names get created as new Organs with source='chimera_generated'."""
    out: dict[str, list[str]] = {}
    for bank, names in (by_bank or {}).items():
        if bank == "degradation":
            continue  # we forbid degradation in chimera output
        ...
        for n in names:
            ...
            hit = (
                db.query(Organ)
                .filter(Organ.bank == bank)
                .filter(func.lower(Organ.name) == n.lower())
                .first()
            )
            if hit:
                ids.append(hit.id)
            else:
                # Create a chimera-generated organ tied to the source cell.
                import re
                slug = re.sub(r"[ /]+", "_", n.lower())
                slug = re.sub(r"[^a-zа-яё0-9_]", "", slug) or "_"
                code = f"{bank}.{slug}_chim_{source_entry_id[-8:]}"
                existing = db.query(Organ).filter(Organ.code == code).first()
                if existing:
                    ids.append(existing.id)
                else:
                    o = Organ(
                        bank=bank, code=code, name=n,
                        description="generated by chimera_weaver",
                        source="chimera_generated", source_entry_id=source_entry_id,
                    )
                    db.add(o)
                    db.flush()
                    ids.append(o.id)
...
@router.post("/entries/{entry_id}/generate-chimera-draft")
def generate_chimera_draft(
    entry_id: str,
    payload: ChimeraGenRequest,
    db: Session = Depends(get_db),
    ...
    if not e:
        raise HTTPException(404, "no such corpus entry")
    if e.kind != "chimera":
        raise HTTPException(400, "draft generation requires kind=chimera")

    # Build the canonical organ menu for the weaver to choose from.
    rows = db.query(Organ).filter(Organ.source == "canon_v0.1").order_by(Organ.bank, Organ.name).all()
    canon: dict[str, list[str]] = {}
    for o in rows:
        canon.setdefault(o.bank, []).append(o.name)

    provider = get_provider()
    out = provider.call_role(
        "chimera_weaver",
        {"chimera_title": e.title, "chimera_body": e.body_md, "canon_organs_by_bank": canon},
        model=payload.model,
    )
    spec = out.pop("_prompt_spec", None)
    out.pop("_role", None)
    ...
    organs_by_bank_ids = _resolve_organ_names_to_ids(db, out.get("organs") or {}, e.id)

    draft = GenomeDraft(
        name=out.get("name") or f"chimera from {e.code}",
        function=out.get("function"),
        verb=out.get("verb"),
        maturity_stage=max(0, min(5, int(out.get("maturity_stage") or 1))),
        selected_organs=organs_by_bank_ids,
        weaver_verdict={
            "critique": out.get("critique"),
            "source_chimera_id": e.id,
            "source_chimera_code": e.code,
        },
        player_token=x_player_token,
    )
    db.add(draft)
    ...
    return {
        ...
        "selected_organs": draft.selected_organs,
        "critique": out.get("critique"),
        "source_chimera_id": e.id,
    }
